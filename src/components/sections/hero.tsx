...
const stats = {
  ar:[{v:"1,000+",l:"سرير تحت الإدارة"},{v:"24/7",l:"تشغيل وأمن مستمر"},{v:"3 وجبات",l:"يومياً لكل ساكن"},{v:"100%",l:"أسطول بتتبع GPS"}],
  en:[{v:"1,000+",l:"Beds Under Management"},{v:"24/7",l:"Operations & Security"},{v:"3 Meals",l:"Daily Per Resident"},{v:"100%",l:"GPS-Tracked Fleet"}],
};
...
export default function Hero() {
  ...
  return (
    <section ref={ref} style={{ position:"relative", height:"100vh", minHeight:720, display:"flex", flexDirection:"column", overflow:"hidden" }}>
      ...
      {/* Gradient overlay */}
      <div style={{ position:"absolute", inset:0, background:"linear-gradient(160deg, rgba(1,85,48,0.82) 0%, rgba(0,0,0,0.55) 45%, rgba(1,85,48,0.70) 100%)" }}/>

      {/* Animated light particles — desktop only */}
      {!isMobile && [...Array(5)].map((_,i) => (
        <motion.div key={i}
          style={{ position:"absolute", borderRadius:"50%", background:"radial-gradient(circle, rgba(235,185,104,0.18) 0%, transparent 70%)", width:`${180+i*60}px`, height:`${180+i*60}px`, top:`${10+i*15}%`, left:`${5+i*18}%`, pointerEvents:"none" }}
      ...
      <div style={{
        ...
        <motion.div
          ...
          style={{ display:"inline-flex", alignItems:"center", gap:8, marginBottom:22, padding: isMobile ? "6px 14px" : "8px 20px", borderRadius:999, background:"rgba(235,185,104,0.15)", border:"1px solid rgba(235,185,104,0.45)" }}>
          <motion.span
            animate={{ scale:[1,1.5,1], opacity:[1,0.5,1] }} transition={{ duration:2.2, repeat:Infinity }}
            style={{ width:6, height:6, borderRadius:"50%", background:"#ebb968", display:"block", flexShrink:0 }}/>
          <span style={{ fontFamily:"Almarai", fontWeight:700, fontSize: isMobile ? 10 : 12, color:"#ebb968", letterSpacing: isMobile ? "0.05em" : "0.1em" }}>
            {lang==="ar" ? "شريكك الاستراتيجي في خدمات العمالة" : "YOUR STRATEGIC WORKFORCE PARTNER"}
          </span>
        </motion.div>
        ...
        <motion.h1
          ...
          {lang==="ar"
            ?<>بُنيت للناس الذين<br/><span style={{ color:"#ebb968" }}>يبنون المملكة</span></>
            :<>Built for the People<br/><span style={{ color:"#ebb968" }}>Building the Kingdom</span></>}
        </motion.h1>
        ...
        <motion.div
          ...
          <Link href="/#contact"
            style={{ fontFamily:"Almarai", fontWeight:800, fontSize: isMobile ? 14 : 15, textDecoration:"none", padding: isMobile ? "13px 26px" : "15px 36px", borderRadius:16, color:"#1a1a1a", background:"linear-gradient(135deg,#ebb968,#d4a040)", boxShadow:"0 8px 32px rgba(235,185,104,0.48)", transition:"all 0.25s" }}
            onMouseEnter={e=>{(e.currentTarget as HTMLElement).style.transform="translateY(-2px)";(e.currentTarget as HTMLElement).style.boxShadow="0 14px 42px rgba(235,185,104,0.6)";}}
            onMouseLeave={e=>{(e.currentTarget as HTMLElement).style.transform="translateY(0)";(e.currentTarget as HTMLElement).style.boxShadow="0 8px 32px rgba(235,185,104,0.48)";}}>
        ...
        <div style={{ maxWidth:1280, margin:"0 auto", padding: isMobile ? "0 16px" : "0 32px", display:"grid", gridTemplateColumns: isMobile ? "repeat(2,1fr)" : "repeat(4,1fr)" }}>
          {S.map((s,i) => (
            <div key={i} style={{
              ...
              <div style={{ fontFamily:"Almarai", fontWeight:800, fontSize: isMobile ? 20 : 28, color:"#ebb968", lineHeight:1 }}>{s.v}</div>
